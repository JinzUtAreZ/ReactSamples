...
const rowSelect = [
  createSelect('s1', 'Pending'),
  createSelect('s2', 'In Progress'),
  createSelect('s3', 'For Approval'),
  createSelect('s4', 'Completed'),
  createSelect('s5', 'BackJob'),
];
...
export default function SimpleTable() {
  ...
  const [selStateRows, setSelStateRows] = React.useState({});
          ...
          {(rowsPerPage > 0
            ...
            <TableRow key={row.name}>
              ...
              <TableCell style={{ width: 160 }} align="right">
                ...
                >
                  {rowSelect.map((sel) => (
                    <option value={sel.id}>{sel.status}</option>
                  ))}
                  {/* Status  */}
                </select>
              </TableCell>
